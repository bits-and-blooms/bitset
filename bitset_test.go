...
func TestSetIndicesGenerator(t *testing.T) {
	length := uint(256)
	bitSet := New(length)
	...
	defer close(preempt)
	for idx := range bitSet.GenIndices(preempt) {
...
func TestSetIndicesGeneratorWPreempt(t *testing.T) {
	length := uint(256)
	bitSet := New(length)
	...
	if len(indices) != 1 {
		...
		got := indices[0]
		expected := expectedToBeSet[0]
		if got != expected {
			t.Errorf("Unexpected 'Set' bit at index 0. Expected %d, got %d", expected, got)
		}
	}
}
